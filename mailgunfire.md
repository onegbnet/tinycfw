...
- **Attachments** — multiple files, 20 MB total cap (stored in KV alongside the sent record, expires together via `TTL`)
...
- **附件** — 支持多文件，总大小 ≤ 20 MB（与已发送记录一起存入 KV，随 `TTL` 一同过期）
...
- **附件** — 支援多檔案，總大小 ≤ 20 MB（與已傳送紀錄一同儲存於 KV，依 `TTL` 一起過期）
