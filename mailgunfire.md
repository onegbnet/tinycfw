...
A ready-to-run image is published at `ghcr.io/onegbnet/mailgunfire`:

```sh
cp .env.example .env       # set DOMAIN + KEY (your Mailgun account)
docker compose up -d       # → http://localhost:8080
```

AI compose stays off until you point `OLLAMA` at an Ollama endpoint (your own, or the bundled one via `docker compose --profile ai up -d`). Full instructions ship with the image.
...
开箱即用的镜像发布在 `ghcr.io/onegbnet/mailgunfire`：

```sh
cp .env.example .env       # 填入 DOMAIN + KEY（你的 Mailgun 账号）
docker compose up -d       # → http://localhost:8080
```

AI 撰写默认关闭，直到你把 `OLLAMA` 指向一个 Ollama 端点（你自己的，或通过 `docker compose --profile ai up -d` 启动内置的）。完整说明随镜像附带。
...
開箱即用的映像發佈在 `ghcr.io/onegbnet/mailgunfire`：

```sh
cp .env.example .env       # 填入 DOMAIN + KEY（你的 Mailgun 帳號）
docker compose up -d       # → http://localhost:8080
```

AI 撰寫預設關閉，直到你把 `OLLAMA` 指向一個 Ollama 端點（你自己的，或透過 `docker compose --profile ai up -d` 啟動內建的）。完整說明隨映像附帶。
