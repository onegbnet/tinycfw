...
- **Flexible auth** — per-link password via `X-Password`, or global admin password via `X-Admin` / `Bearer`; private deploys can skip authentication entirely
...
### For administrators (with admin password)

- **Global admin password** — manage any link regardless of its modification password; rotate it anytime via the `ADMIN` environment variable
...
| `POST`   | `/_a/:slug`                   | Submit `access`; on success: set unlock cookie + 303 to `/:slug` (regardless of slug kind — URL or file). The subsequent `GET /:slug` dispatches per `redirectMode` |
| `POST`   | `/_admin/auth`                | Browser-only: exchange admin password for HttpOnly `shul_admin` cookie           |
...
Owner-visible (admin password or correct `X-Password`) GET-style responses include `maxHits`, `hits`, `hitsLeft = max(0, maxHits - hits)`, `ttl`, `expiresAt` (Unix seconds), and `expiresInSec`. Unauthenticated / wrong-password responses omit these six fields entirely.
...
`access` still works for multi-file slugs — the password gate page is independent of the landing customization.
...
3. (Optional) Set **environment variables**:
   ...
   |-----------|--------|-------------------------------------------------------------------------------------------|
   | `ADMIN` | Secret | Comma-separated admin passwords; enables admin mode; omit for open access                        |
...
**Admin Password** (required only when `ADMIN` environment variable is configured):
...
or
```
Authorization: Bearer your-admin
```
...
**Browser admin path.** The web UI doesn't keep the admin password in JS storage — instead it exchanges the password for an HttpOnly `shul_admin` cookie via `POST /_admin/auth`. Subsequent admin actions ride on that cookie. API clients (machine-to-machine) keep using `X-Admin` / `Bearer` headers as documented above; the cookie path is browser-only and complementary.
...
|-------------------------|--------|----------------------------------------------------|
| `UNAUTHORIZED`          | 401    | Missing or invalid admin password                       |
...
| `UPLOAD_TOKEN_INVALID` | 403    | `X-Upload` missing or doesn't match the active session             |
...
| `X-Admin`  | If ADMIN set | Admin password                          |
...
| 401    | Admin password missing or invalid                   |
...
Create multiple short links in one request by sending a JSON array instead of a single object. Requires admin password.

**Headers:**

| Header       | Required | Description                          |
|--------------|----------|--------------------------------------|
| `X-Admin`  | Yes      | Admin password                            |
...
Note: admins with `X-Admin` can delete any slug without knowing its modification password.
...
Delete **all** short links in the KV namespace. Requires admin password. Use with caution.
...
The following operations are exclusive to holders of the admin password (`X-Admin`):
...
| `X-Admin`  | If `ADMIN` set    | Admin password (skips `X-Password` and rate limit)       |
...
| `redirectMode`    | string   | No       | `instant` or `manual`; default `instant`                                          |
| `countdown` / `redirectPageTitle` / `redirectPageContent` / `manualBtnTitle` / `darkBackground` / `centerContent` / `maxHits` / `access` / `ttl` | — | No | Same semantics as URL slug create; applied at commit. **Multi-file slugs silently normalize away** all landing customization (`redirectMode='manual'`, `permanent=true`, `countdown=0`, `manualBtnTitle/redirectPageTitle/redirectPageContent=null`, `darkBackground/centerContent=false`) — the file-list landing is fixed. See "Multi-file landing page" below. |
...
| Header           | Required | Description                                |
|------------------|----------|--------------------------------------------|
| `X-Upload` | Yes      | Token from reserve                         |
...
**Auth (only when `access` is set on the slug):**

- API clients: send `X-Password: <password>`.
...
**Errors:** 403 if `access` is required and neither cookie nor correct `X-Password` is supplied. Slug-not-found falls back to the same `DEFAULT`/home redirect as ordinary `GET /:slug`.
...
For **file slugs**, this endpoint serves the file-list / download page instead of redirecting (single-file slugs in `instant` mode without `access` stream the file directly). When `access` is set, browsers see a password gate that posts to `POST /_a/:slug` and stores an HttpOnly unlock cookie on success — file links on the page never carry the password in the URL. API clients should bypass the page and call `GET /:slug?__f=1&i=<idx>` directly with `X-Password`.
...
- **访问密码保护** — 为任意链接（URL 或文件，直接跳转或带跳转页面，单文件或多文件）设置 `access`。访客先看到独立的密码页面，验证通过后才进入链接本身的行为——直接跳转直接 301 到目标、带跳转页面则显示设定的跳转页、文件 slug 直接传文件。验证通过 HttpOnly cookie 持久化（多文件下载会话不会反复要求输入）；失败的密码尝试不消耗有效次数；密码不会出现在任何 URL 中
...
- **三个正交的限定选项** — 每个链接可叠加任意组合：(a) **有效时长**（`ttl`，60 秒到 12 个月，绝对过期时间——修改时不会滑动过期窗口）、(b) **有效次数**（`maxHits`，N 次后自毁——`maxHits: 1` 对应经典"一次性链接"）、(c) **访问密码**（`access`，服务端门禁，访客需通过验证才进入内容）。UI 将三者归入「限定选项」面板，自由组合。次数采用"打开页面"语义 + 15 分钟同访客 cookie 去重；密码门禁失败不消耗配额
...
- **灵活认证** — 短链密码（`X-Password`）或全局管理密码（`X-Admin` / `Bearer`）；私有部署可完全跳过认证
...
- **全局管理密码** — 可管理任意链接，无需其短链密码；随时通过 `ADMIN` 环境变量轮换管理密码
...
| `POST`   | `/_a/:slug`                   | 提交 `access`；成功 → set 解锁 cookie + 303 跳回 `/:slug`（不分 URL/文件 slug）；后续 GET 按 `redirectMode` 分发 |
...
`access` 在多文件短码上仍然有效——密码门禁页跟 landing 定制完全独立。
...
3. （可选）设置**环境变量**：
   ...
   | 变量名    | 类型   | 说明                                                                          |
   |-----------|--------|-------------------------------------------------------------------------------|
   | `ADMIN` | Secret | 逗号分隔的管理密码；不设则无需认证                                           |
...
**浏览器管理员路径**：Web 界面不在 JS storage 里保留管理密码，而是通过 `POST /_admin/auth` 把管理密码换成 HttpOnly `shul_admin` cookie，后续管理操作均凭该 cookie。API 客户端（机器到机器）仍使用 `X-Admin` / `Bearer` 请求头；cookie 路径仅服务于浏览器，二者互补。
...
| `UPLOAD_TOKEN_INVALID`    | 403    | `X-Upload` 缺失或与活跃会话不匹配                                    |
...
| `access`     | string  | 否   | 访问密码——在**所有**跳转模式与目标类型（URL/单文件/多文件）下都生效。3–16 个可打印非空格字符。服务端密码门禁页（独立于跳转页）在所有内容之前；访客必须通过门禁后才进入 `redirectMode` 分发逻辑。PUT 传空字符串可清除已设置的密码。 |
...
注：持有 `X-Admin` 的管理员可删除任意短码，无需其短链密码。
...
| `X-Admin`  | 配置时必填   | 管理密码（可绕过 `X-Password` 与频率限制）        |
...
| `redirectMode`    | string   | 否     | `instant` 或 `manual`；默认 `instant`                                       |
| `countdown` / `redirectPageTitle` / `redirectPageContent` / `manualBtnTitle` / `darkBackground` / `centerContent` / `maxHits` / `access` / `ttl` | — | 否 | 与 URL 短码创建语义相同，提交时应用。 |
...
**鉴权（仅当短码设置了 `access` 时需要）：**

- API 客户端：发送 `X-Password: <password>`
...
**错误码：** 当 `access` 必需但 cookie 与 `X-Password` 都未提供（或错误）时返回 403。短码不存在则与普通 `GET /:slug` 一样回退到 `DEFAULT` / 首页跳转。
...
对**文件短码**，此端点不再跳转，而是渲染文件列表 / 下载页面（单文件、`instant` 模式、未设 `access` 的情况下直接流式下载文件）。设置了 `access` 时，浏览器看到密码门禁，提交后 POST 到 `/_a/:slug`，成功后服务端设置 HttpOnly 解锁 cookie —— 页面上的文件链接绝不在 URL 中携带密码。API 客户端应跳过此页面，直接用 `X-Password` 调 `GET /:slug?__f=1&i=<idx>`。
...
- **存取密碼保護** — 為任意連結（URL 或檔案，直接跳轉或帶跳轉頁面，單檔或多檔）設定 `access`。訪客先看到獨立的密碼頁面，驗證通過後才進入連結本身的行為——直接跳轉直接 301 到目標、帶跳轉頁面則顯示設定的跳轉頁、檔案 slug 直接傳檔案。驗證通過 HttpOnly cookie 持久化（多檔下載工作階段不會反覆要求輸入）；失敗的密碼嘗試不消耗有效次數；密碼不會出現在任何 URL 中
...
- **三個正交的限定選項** — 每個連結可疊加任意組合：(a) **有效時長**（`ttl`，60 秒到 12 個月，絕對過期時間——修改時不會滑動過期視窗）、(b) **有效次數**（`maxHits`，N 次後自毀——`maxHits: 1` 對應經典「一次性連結」）、(c) **存取密碼**（`access`，伺服端門禁，訪客需通過驗證才進入內容）。UI 將三者歸入「限定選項」面板，自由組合。次數採用「打開頁面」語意 + 15 分鐘同訪客 cookie 去重；密碼門禁失敗不消耗配額
...
- **靈活認證** — 短鏈密碼（`X-Password`）或全域管理密碼（`X-Admin` / `Bearer`）；私有部署可完全跳過認證
...
- **全域管理密碼** — 可管理任意連結，無需其短鏈密碼；隨時透過 `ADMIN` 環境變數輪換管理密碼
...
| `POST`   | `/_a/:slug`                   | 提交 `access`；成功 → set 解鎖 cookie + 303 跳回 `/:slug`（不分 URL/檔案 slug）；後續 GET 按 `redirectMode` 分發 |
...
`access` 在多檔案短碼上仍然有效——密碼門禁頁跟 landing 客製化完全獨立。
...
3. （選用）設定**環境變數**：
   ...
   | 變數名    | 類型   | 說明                                                                          |
   |-----------|--------|-------------------------------------------------------------------------------|
   | `ADMIN` | Secret | 逗號分隔的管理密碼；不設則無需認證                                           |
...
**瀏覽器管理員路徑**：Web 介面不在 JS storage 中保留管理密碼，而是透過 `POST /_admin/auth` 將管理密碼換成 HttpOnly `shul_admin` cookie，後續管理操作均憑該 cookie。API 客戶端（機器對機器）仍使用 `X-Admin` / `Bearer` 請求標頭；cookie 路徑僅服務於瀏覽器，兩者互補。
...
| `UPLOAD_TOKEN_INVALID`    | 403    | `X-Upload` 缺失或與作用中工作階段不符                                |
...
| `access`     | string  | 否   | 存取密碼——在**所有**跳轉模式與目標類型（URL/單檔/多檔）下都生效。3–16 個可列印非空格字元。伺服端密碼門禁頁（獨立於跳轉頁）在所有內容之前；訪客必須通過門禁後才進入 `redirectMode` 分發邏輯。PUT 傳空字串可清除已設定的密碼。 |
...
注：持有 `X-Admin` 的管理員可刪除任意短碼，無需其短鏈密碼。
...
| `X-Admin`  | 設定時必填   | 管理密碼（可繞過 `X-Password` 與頻率限制）        |
...
| `redirectMode`    | string   | 否     | `instant` 或 `manual`；預設 `instant`                                       |
| `countdown` / `redirectPageTitle` / `redirectPageContent` / `manualBtnTitle` / `darkBackground` / `centerContent` / `maxHits` / `access` / `ttl` | — | 否 | 與 URL 短碼建立語意相同，提交時套用。 |
...
**鑑權（僅當短碼設定了 `access` 時需要）：**

- API 客戶端：發送 `X-Password: <password>`
...
**錯誤碼：** 當 `access` 必須但 cookie 與 `X-Password` 都未提供（或錯誤）時回傳 403。短碼不存在則與普通 `GET /:slug` 一樣回退到 `DEFAULT` / 首頁跳轉。
...
對**檔案短碼**，此端點不再跳轉，而是渲染檔案列表 / 下載頁面（單檔、`instant` 模式、未設 `access` 的情況下直接串流下載檔案）。設定了 `access` 時，瀏覽器看到密碼閘門，提交後 POST 至 `/_a/:slug`，成功後伺服器設定 HttpOnly 解鎖 cookie —— 頁面上的檔案連結絕不在 URL 中攜帶密碼。API 客戶端應跳過此頁面，直接以 `X-Password` 呼叫 `GET /:slug?__f=1&i=<idx>`。
